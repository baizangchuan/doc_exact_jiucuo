...
# doc_type='Death_Info'
# doc_type='Case_Info'
doc_type='Preoperative_Summary_Info'

rawSeg_Mark="rawSeg"
normResult_Mark="normResult"
key_metrics_flag=0
input_json_dir=f"/data1/hhj/project/demo/template/{doc_type}/input.json"
format_guide_json=f"/data1/hhj/project/demo/template/{doc_type}/output_example.json"#格式参考文件的路径
final_data_save_dir=f"/data1/hhj/project/demo/template/{doc_type}/final_data_py.json"
...
def get_rawSeg_key_set_from_format_guide_json(format_guide_json_dir):#从目标输出中提取rawSeg_key_set，即所有待匹配的关键词
    with open(format_guide_json_dir, 'r', encoding='utf-8') as f:
        data = json.load(f)
        rawSeg = data['data']['result'][rawSeg_Mark]
    rawSeg_key_set=[]
...
def get_normResult_guide_from_format_guide_json(format_guide_json):#从目标输出文件中，找出normResult格式参考列表
    with open(format_guide_json, 'r', encoding='utf-8') as f:
        data = json.load(f)
        normResult = data['data']['result'][normResult_Mark]
    normResult_guide=[]
...
def main():
    ...
    #保存
    with open(final_data_save_dir, 'w') as json_file:
