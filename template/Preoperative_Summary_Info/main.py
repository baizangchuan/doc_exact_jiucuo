'''
术前小结，是最基础款
rawSeg=rawSeg，normResult=normResult
'''
...
def get_rawSeg_key_set_from_format_guide_json(format_guide_json_dir):#从目标输出中提取rawSeg_key_set，即所有待匹配的关键词
    with open(format_guide_json_dir, 'r', encoding='utf-8') as f:
        data = json.load(f)
        rawSeg = data['data']['result']['rawSeg']
    rawSeg_key_set=[]
...
def get_normResult_guide_from_format_guide_json(format_guide_json):#从目标输出文件中，找出normResult格式参考列表
    with open(format_guide_json, 'r', encoding='utf-8') as f:
        data = json.load(f)
        normResult = data['data']['result']['normResult']
    normResult_guide=[]
...
def gen_final_data(normResult,rawSeg):
  ...
  "data": {
      "result": {
          ...
          "recordType": "preoperative_summary_info",
          "normResSubSeg": {},
          "rawSeg": rawSeg
          }
  }
...
def main():
    ...
    input_json_dir="/data1/hhj/project/demo/template/Preoperative_Summary_Info/input.json"
    format_guide_json="/data1/hhj/project/demo/template/Preoperative_Summary_Info/output_example.json"#格式参考文件的路径
    ...
    #保存
    with open("/data1/hhj/project/demo/template/Preoperative_Summary_Info/final_data_py.json", 'w') as json_file:
